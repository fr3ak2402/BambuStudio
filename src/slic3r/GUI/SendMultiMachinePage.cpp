...
void SendDeviceItem::doRender(wxDC& dc)
{
    ...
    //device state
    if (state_printable <= 2) {
        dc.SetTextForeground(wxColour(105, 58, 113));
    }
    else {
    ...
    if (m_hover) {
        dc.SetPen(wxPen(wxColour(105, 58, 113)));
...
wxPanel* SendMultiMachinePage::create_page()
{
    ...
    // add send button
    btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour(69, 43, 74), StateColor::Pressed), std::pair<wxColour, int>(wxColour(153, 102, 166), StateColor::Hovered),
        std::pair<wxColour, int>(wxColour(105, 58, 113), StateColor::Normal));
